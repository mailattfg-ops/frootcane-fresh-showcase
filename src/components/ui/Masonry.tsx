...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const imageKey = useMemo(() => items.map((item) => item.img).join("|"), [items]);
  const [loadedFor, setLoadedFor] = useState("");
  const imagesReady = loadedFor === imageKey;
  ...
  return (
    <div ref={containerRef} className="relative w-full" style={{ height: contentHeight ? `${contentHeight}px` : undefined }}>
      {grid.map((item) => (
        <div
          key={item.id}
          data-key={item.id}
